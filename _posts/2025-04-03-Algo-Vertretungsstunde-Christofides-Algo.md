...
class NearestNeighborAnimation {
    ...
    drawSelectionStep(stepIndex) {
        ...
        step.candidates.forEach(candidate => {
            ...
            // Hier wird die Farbe für nicht-ausgewählte Kanten auf blau gesetzt
...
function nnShowFullTable() {
    ...
    nnAnimation.selectionSteps.forEach((step, stepIndex) => {
        step.candidates.forEach(candidate => {
            // Füge die CSS-Klasse 'selected' hinzu, wenn es der nächste Nachbar ist
...
function nnNextStep() {
    ...
    // Wenn wir bereits am Ende sind, zeichnen wir erneut den finalen Pfad
    if (nnAnimation.currentPathIndex > nnAnimation.selectionSteps.length) {
    ...
    if (nnAnimation.currentPathIndex > nnAnimation.selectionSteps.length) {
        ...
        // Zusätzlich sicherstellen, dass die Gesamtlänge angezeigt wird
        let totalLength = 0;
        for (let i = 1; i < nnAnimation.nnPath.length; i++) {
            totalLength += nnAnimation.distance(
                nnAnimation.nodes[nnAnimation.nnPath[i - 1]],
        ...
        // Info-Panel mit finaler Route und Gesamtlänge aktualisieren
        nnUpdateInfoPanel();
